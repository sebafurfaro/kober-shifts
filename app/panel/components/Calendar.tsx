...
export function Calendar() {
  ...
  async function loadInitialData() {
    try {
      const [patientsRes, professionalsRes, locationsRes, specialtiesRes] = await Promise.all([
        ...
        fetch("/api/admin/professionals").catch(() => null),
        fetch("/api/admin/locations").catch(() => null),
        fetch("/api/admin/specialties").catch(() => null),
      ]);
  ...
  const loadEvents = useCallback(async (start: Date, end: Date, forceReload = false) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/appointments?start=${startStr}&end=${endStr}`);
